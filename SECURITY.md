...
StrixPDF now uses nginx-unprivileged for enhanced security. This follows the Principle of Least Privilege and is essential for production environments.
...
```bash
docker build -t strixpdf .
docker run -p 8080:8080 strixpdf
```
...
# Run the container
docker run -p 8080:8080 strixpdf-simple
```
